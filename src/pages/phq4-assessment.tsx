...
export default function PHQ4Assessment() {
  ...
  const getLocalizedText = (textObj: any, property: string, fallback: string = '') => {
    const lang = getCurrentLanguage();
    if (textObj[lang] && textObj[lang][property]) {
      return textObj[lang][property];
    }
    if (textObj.en && textObj.en[property]) {
      return textObj.en[property];
    }
    return fallback;
  };

  const getLocalizedOption = (option: any, fallback: string = '') => {
    const lang = getCurrentLanguage();
    return option[lang] || option.en || fallback;
  };
  ...
  const startVoiceInput = () => {
    ...
    recognitionRef.current.onresult = (event: any) => {
      ...
      const voiceCommands: Record<string, Record<string, string[]>> = {
        en: { '0': ['not at all', 'never', 'zero'], '1': ['several days', 'sometimes', 'one'], '2': ['more than half', 'often', 'two'], '3': ['nearly every day', 'always', 'three'] },
        zh: { '0': ['完全没有', '从不'], '1': ['几天', '有时'], '2': ['超过一半', '经常'], '3': ['几乎每天', '总是'] },
        bn: { '0': ['একদমই না', 'না'], '1': ['কয়েক দিন', 'কখনো কখনো'], '2': ['অর্ধেকের বেশি', 'প্রায়ই'], '3': ['প্রায় প্রতিদিন', 'সবসময়'] },
        ta: { '0': ['சற்றும் இல்லை', 'இல்லை'], '1': ['சில நாட்கள்', 'சில சமயம்'], '2': ['பாதி நাட்களுக்கு மேல்', 'அடிக்கடி'], '3': ['கிட்டத்தட்ட தினமும்', 'எப்போதும்'] },
        my: { '0': ['လုံးဝမရှိ', 'မရှိ'], '1': ['ရက်သတ္တပတ်အချို့', 'တစ်ခါတလေ'], '2': ['ရက်ထက်ဝက်ကျော်', 'မကြာခဏ'], '3': ['နေ့စဉ်နီးပါး', 'အမြဲ'] },
        idn: { '0': ['tidak sama sekali', 'tidak'], '1': ['beberapa hari', 'kadang'], '2': ['lebih dari setengah', 'sering'], '3': ['hampir setiap hari', 'selalu'] }
      };
  ...
  const generateRecommendations = (total: number, depression: number, anxiety: number, risk: string): string[] => {
    ...
    const recTexts: Record<string, Record<string, string[]>> = {
      en: {
        minimal: [
      ...
      zh: {
        minimal: ['继续保持当前的自我护理习惯', '考虑定期锻炼和健康的生活方式', '与支持你的朋友和家人保持联系'],
        mild: ['考虑压力管理技巧，如冥想或深呼吸', '保持规律的睡眠时间和健康饮食', '联系同伴支持小组获得额外支持'],
        moderate: ['考虑与心理健康专业人士交谈', '探索咨询或治疗选择', '练习日常减压活动'],
        severe: ['立即寻求专业心理健康支持', '如需要请联系危机支持服务', '考虑医疗评估和治疗选择']
      }
      // Add other languages as needed
    };

    const currentRecTexts = recTexts[lang] || recTexts.en;
    return currentRecTexts[risk as keyof typeof currentRecTexts] || currentRecTexts.minimal;
  };
